...
`sippy_serve` and `sippy_ng_start` spawn detached processes. A second start is skipped if a matching process is already running, but the tool still polls the HTTP endpoint to confirm the API is actually accepting requests before reporting ready. If the process is detected but the API does not respond within the timeout, the tool reports an error with guidance to restart.
