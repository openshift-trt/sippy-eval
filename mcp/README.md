...
`sippy_serve` and `sippy_ng_start` spawn detached processes. A second start is refused if a matching process is already running (see `server.py` for detection logic). `sippy_serve` verifies the HTTP API is accepting requests (via `GET /api`) before reporting ready — both when spawning a new process and when detecting an existing one. If the API doesn't respond within `ready_timeout` (default 120s), the tool returns an error instead of reporting success.
