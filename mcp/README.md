...
`sippy_serve` and `sippy_ng_start` spawn detached processes. A second start is skipped if a matching process is already running, but the tool still polls the HTTP endpoint to verify the API is accepting requests before reporting ready.
