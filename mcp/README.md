...
MCP server unit tests live in `test_server.py` and are run by `make test` alongside Go and JS tests.
...
Test dependencies are declared in `requirements-dev.txt` (extends `requirements.txt` with `pytest`).
