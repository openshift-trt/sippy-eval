...
`sippy_serve` and `sippy_ng_start` spawn detached processes. If a matching process is already running, the tool verifies the HTTP endpoint is responding before reporting ready. If the process exists but the API is not yet available (e.g. during `go run` compilation), the tool polls until the endpoint responds or a 120-second timeout elapses.
