...
`sippy_serve` and `sippy_ng_start` spawn detached processes. If a matching process is already running, the tool polls the HTTP endpoint to verify the API is actually accepting requests before reporting ready (see `server.py` for detection and readiness logic).
