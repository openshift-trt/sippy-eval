...
`sippy_serve` and `sippy_ng_start` spawn detached processes. A second start is refused if a matching process is already running (see `server.py` for detection logic). When an existing `sippy_serve` process is detected, the tool polls the HTTP endpoint to verify the API is actually accepting requests before reporting ready.
