...
`sippy_serve` and `sippy_ng_start` spawn detached processes. A second start is refused if a matching process is already running; in that case the tool polls the HTTP endpoint to verify the API is actually accepting requests before reporting ready. If the process is running but the API is not responding (e.g. during Go compilation), the tool waits up to 120 seconds and returns a clear error if the API never becomes available.
