...
`sippy_serve` and `sippy_ng_start` spawn detached processes. A second start is refused if a matching process is already running (see `server.py` for detection logic). Both tools verify HTTP readiness by polling the listen address before reporting success — this applies whether the process was just spawned or was already running.
