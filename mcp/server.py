...
async def _poll_ready(url: str, timeout: int, pids: list[int]) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse.

    Checks that at least one PID in *pids* is still alive each iteration.
...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _poll_ready(host_hint, 120, existing)
            if err:
                return (
                    f"sippy_serve already running (pid(s) {pids}) but API not ready: {err}. "
