...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready(host_hint, 120, pids=existing)
            if err:
                return (
                    f"sippy_serve process detected (pid(s) {pids_str}) but API {err}. "
                    f"log: {log_path}. Try restart=True to restart."
                )
            return (
                f"sippy_serve already running and ready (pid(s) {pids_str}). Listen: {host_hint} "
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready(ng_url, 120, pids=existing)
            if err:
                return (
                    f"sippy_ng_start process detected (pid(s) {pids_str}) but {err}. "
                    f"log: {log_path}. Try restart=True to restart."
                )
            return (
                f"sippy_ng_start already running and ready (pid(s) {pids_str}). "
                f"URL: {ng_url} log: {log_path}. "
...
async def _wait_for_ready(
    ...
    *,
    ...
    Optionally checks that the process is still alive via *proc* (Popen) or *pids*.
    """
...
async def _spawn_background(
    ...
    if ready_url:
        err = await _wait_for_ready(ready_url, ready_timeout, proc=proc)
        if err:
