...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            if err:
                return (
                    f"sippy_serve process detected (pid(s) {pid_str}) but API is not available: "
                    f"{err}. log: {log_path}. Call with restart=True to restart."
                )
            return (
                f"sippy_serve already running (pid(s) {pid_str}). Listen: {host_hint} "
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            pid_str = ", ".join(str(p) for p in existing)
            err = await _wait_for_ready("http://127.0.0.1:3000/sippy-ng", 120, pids=existing)
            if err:
                return (
                    f"sippy_ng_start process detected (pid(s) {pid_str}) but not serving: "
                    f"{err}. log: {log_path}. Call with restart=True to restart."
                )
            return (
                f"sippy_ng_start already running (pid(s) {pid_str}). "
...
async def _wait_for_ready(
    ...
    while loop.time() < deadline:
        if proc is not None:
            ...
            if not any(_pid_is_alive(p) for p in pids):
