...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready(host_hint, 120, existing[0])
            if err:
                return (
                    f"sippy_serve process detected (pid(s) {pids}) but API not responding: "
                    f"{err}. Log: {log_path}. Try restart=True or check the log for errors."
                )
            return (
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready(ready_url, 120, existing[0])
            if err:
                return (
...
async def _wait_for_ready(url: str, timeout: int, pid: int) -> str | None:
...
async def _spawn_background(
    ...
    if ready_url:
        err = await _wait_for_ready(ready_url, ready_timeout, proc.pid)
        if err:
