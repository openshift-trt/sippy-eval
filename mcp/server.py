...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready_pid(host_hint, 120, existing)
            if err:
                return (
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready_pid(ready_url, 120, existing)
            if err:
                return (
...
async def _wait_for_ready_pid(url: str, timeout: int, pids: list[int]) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse, checking *pids* liveness.

    Like ``_wait_for_ready`` but works with already-running processes identified by PID
    rather than a ``Popen`` handle.
    """
    loop = asyncio.get_running_loop()
    deadline = loop.time() + timeout
    while loop.time() < deadline:
