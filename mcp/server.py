...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            pids = ", ".join(str(p) for p in existing)
            err = await _wait_for_ready("http://127.0.0.1:3000/sippy-ng", 120, pid=existing[0])
            if err:
                tail = _tail_file(log_path, 40)
                return (
                    f"sippy_ng_start process detected (pid(s) {pids}) but not serving: "
                    f"{err}. log: {log_path}. Call with restart=True to restart.\n"
                    f"--- tail ---\n{tail}"
                )
            return (
...
async def _wait_for_ready(
    ...
    *,
    ...
    Monitors process liveness via *proc* (newly spawned Popen) or *pid* (existing process).
    """
