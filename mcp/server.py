...
async def _check_url_ready(url: str, pids: list[int], timeout: int = 120) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse.

    Checks that at least one PID in *pids* is still alive between attempts.
...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _check_url_ready(host_hint, existing)
            if err:
                return (
                    f"sippy_serve process detected (pid(s) {pids}) but API {err}. "
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            pids = ", ".join(str(p) for p in existing)
            err = await _check_url_ready(ng_url, existing)
            if err:
                return (
