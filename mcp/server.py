        ...
        if not restart:
            ...
            if err:
                return (
                    ...
                    f"at {host_hint}. The server may still be compiling or starting up. "
        ...
        if not restart:
            ...
            if err:
                return (
                    ...
                    f"at {ng_url}. The dev server may still be starting up. "
                    f"Log: {log_path}. Call with restart=True to restart."
                )
            return (
...
async def _wait_for_ready(url: str, timeout: int, proc: subprocess.Popen) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse, watching for early exit."""
    loop = asyncio.get_running_loop()
    deadline = loop.time() + timeout
    while loop.time() < deadline:
        code = proc.poll()
