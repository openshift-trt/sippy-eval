...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_url(host_hint, _READY_TIMEOUT, existing)
            if err:
                return (
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_url(ng_url, _READY_TIMEOUT, existing)
            if err:
                return (
...
async def _wait_for_url(
    url: str, timeout: int = _READY_TIMEOUT, pids: list[int] | None = None,
) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse.

    If *pids* is given, returns early when none of the processes are alive.
