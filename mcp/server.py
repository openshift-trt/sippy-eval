...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready_pids(host_hint, 120, existing)
            if err:
                return (
                    f"sippy_serve process detected (pid(s) {pids}) but {err}. "
                    f"Listen: {host_hint} log: {log_path}. "
                    f"Call with restart=True to restart, or wait for the server to finish starting."
                )
            return (
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_ready_pids(ng_url, 120, existing)
            if err:
                return (
                    f"sippy_ng_start process detected (pid(s) {pids}) but {err}. "
...
async def _wait_for_ready_pids(url: str, timeout: int, pids: list[int]) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse, checking *pids* for liveness."""
    loop = asyncio.get_running_loop()
    deadline = loop.time() + timeout
    while loop.time() < deadline:
