...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_url(host_hint, timeout=120, pid=existing[0])
            if err:
                return (
                    f"sippy_serve process detected (pid(s) {pids}) but API is not available: "
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _wait_for_url(ready_url, timeout=120, pid=existing[0])
            if err:
                return (
                    f"sippy_ng_start process detected (pid(s) {pids}) but not serving yet: "
                    f"{err}. log: {log_path}. Call with restart=True to restart."
                )
            return (
...
async def _wait_for_url(url: str, timeout: int, pid: int | None = None) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse.

    If *pid* is given, returns early when the process disappears.
