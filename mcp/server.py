...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _poll_url(host_hint)
            if err:
                return (
                    f"sippy_serve already running (pid(s) {pids}) but API not responding "
...
async def sippy_ng_start(
    ...
    if existing:
        if not restart:
            ...
            err = await _poll_url(ng_url)
            if err:
                return (
                    f"sippy_ng_start already running (pid(s) {pids}) but not responding "
                    f"({err}). Typical URL: {ng_url} log: {log_path}. "
                    f"Call with restart=True to restart."
                )
            return (
...
async def _poll_url(url: str, timeout: int = 120) -> str | None:
    """Poll *url* until it returns an HTTP response or *timeout* seconds elapse.

    Returns ``None`` on success, or an error string on timeout.
    """
...
async def _wait_for_ready(url: str, timeout: int, proc: subprocess.Popen) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse, watching *proc* for early exit."""
    loop = asyncio.get_running_loop()
    deadline = loop.time() + timeout
    while loop.time() < deadline:
        code = proc.poll()
