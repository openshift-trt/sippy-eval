...
async def sippy_serve(
    ...
    if existing:
        if not restart:
            ...
            err = await _poll_url_ready(host_hint, 120, pids=existing)
            if err:
                return (
            ...
            return (
                f"sippy_serve already running (pid(s) {pids_str}). Listen: {host_hint} "
                f"log: {log_path}. Call with restart=True to restart."
            )
...
async def _poll_url_ready(url: str, timeout: int, pids: list[int] | None = None) -> str | None:
    """Poll *url* until it responds or *timeout* seconds elapse.

    If *pids* are given, returns early when none of them are alive.
    Returns an error string or ``None`` on success.
    """
    loop = asyncio.get_running_loop()
    deadline = loop.time() + timeout
    while loop.time() < deadline:
        if pids is not None and not any(_pid_alive(p) for p in pids):
