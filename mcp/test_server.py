...
class TestPidExists:
    ...
    def test_nonexistent_pid(self):
        assert _pid_exists(2**22 - 1) is False
...
class TestPollReady:
    def test_returns_none_on_immediate_success(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _poll_ready("http://localhost:8080/api/releases", timeout=5)
            )
    ...
    def test_returns_error_on_timeout(self):
        with mock.patch(
            ...
            result = asyncio.run(
                _poll_ready("http://localhost:8080/api/releases", timeout=2)
            )
    ...
    def test_retries_until_success(self):
        ...
        with mock.patch("server.urllib.request.urlopen", side_effect=_urlopen):
            result = asyncio.run(
                _poll_ready("http://localhost:8080/api/releases", timeout=30)
            )
        assert result is None
        assert call_count == 3
    ...
    def test_ignores_pids_when_none(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _poll_ready("http://localhost:8080/api/releases", timeout=5, pids=None)
            )
        assert result is None
