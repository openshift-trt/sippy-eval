...
class TestPollUrl:
    def test_returns_none_when_url_responds(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _poll_url("http://localhost:9999", timeout=5, pids=[os.getpid()])
            )
        assert result is None

    def test_returns_error_when_process_exits(self):
        with mock.patch("server.urllib.request.urlopen", side_effect=OSError("refused")):
            with mock.patch("server._pid_alive", return_value=False):
                result = asyncio.run(
                    _poll_url("http://localhost:9999", timeout=5, pids=[99999])
                )
    ...
    def test_returns_timeout_when_url_never_responds(self):
        with mock.patch("server.urllib.request.urlopen", side_effect=OSError("refused")):
            result = asyncio.run(
                _poll_url("http://localhost:9999", timeout=1, pids=[os.getpid()])
            )
