...
class TestWaitForUrl:
    ...
    def test_bails_early_when_all_pids_die(self):
        with mock.patch("server._pid_alive", return_value=False):
            with mock.patch(
                "server.urllib.request.urlopen", side_effect=ConnectionRefusedError
            ):
                result = asyncio.run(
                    _wait_for_url("http://127.0.0.1:8080", timeout=30, pids=[99999])
                )
        assert result is not None
        assert "process exited" in result

    def test_continues_when_some_pids_alive(self):
        alive_map = {100: True, 200: False}

        with mock.patch("server._pid_alive", side_effect=lambda p: alive_map.get(p, False)):
            with mock.patch("server.urllib.request.urlopen"):
                result = asyncio.run(
                    _wait_for_url("http://127.0.0.1:8080", timeout=5, pids=[100, 200])
                )
        assert result is None
