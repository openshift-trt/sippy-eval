...
class TestWaitForReady:
    def test_immediate_success(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _wait_for_ready("http://127.0.0.1:9999", timeout=5)
            )
    ...
    def test_timeout_when_url_unreachable(self):
        with mock.patch(
            ...
            result = asyncio.run(
                _wait_for_ready("http://127.0.0.1:9999", timeout=2)
            )
    ...
    def test_proc_exit_detected(self):
        ...
        result = asyncio.run(
            _wait_for_ready("http://127.0.0.1:9999", timeout=5, proc=proc)
        )
    ...
    def test_pids_exit_detected(self):
        ...
        with mock.patch(
            ...
            result = asyncio.run(
                _wait_for_ready("http://127.0.0.1:9999", timeout=5, pids=[dead_pid])
            )
        assert "process(es) exited" in result

    def test_pids_alive_waits_for_url(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _wait_for_ready(
                    "http://127.0.0.1:9999", timeout=5, pids=[os.getpid()]
                )
            )
        assert result is None
