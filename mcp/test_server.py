...
class TestPollUrl:
    def test_returns_none_when_url_responds(self):
        ...
        thread.start()
        try:
            result = asyncio.run(_poll_url(f"http://127.0.0.1:{port}", timeout=5))
    ...
    def test_returns_error_when_url_unreachable(self):
        result = asyncio.run(_poll_url("http://127.0.0.1:19999", timeout=2))
        assert result is not None
        assert "not ready after 2s" in result

    def test_returns_none_when_url_becomes_available(self):
        server = http.server.HTTPServer(("127.0.0.1", 0), _OKHandler)
        port = server.server_address[1]
        server.server_close()

        async def _run():
            async def _delayed_serve():
                await asyncio.sleep(2)
                s = http.server.HTTPServer(("127.0.0.1", port), _OKHandler)
                t = threading.Thread(target=s.serve_forever, daemon=True)
                t.start()
                return s

            task = asyncio.create_task(_delayed_serve())
            result = await _poll_url(f"http://127.0.0.1:{port}", timeout=10)
            s = await task
            s.shutdown()
            s.server_close()
            return result
