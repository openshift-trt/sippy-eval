...
class TestWaitForReadyPids:
    ...
    def test_returns_error_when_all_pids_dead(self):
        dead_pid = 2**22 - 1
        result = asyncio.run(
            _wait_for_ready_pids("http://127.0.0.1:8080", 5, [dead_pid])
        )
    ...
    def test_returns_timeout_error(self):
        with mock.patch(
            ...
            result = asyncio.run(
                _wait_for_ready_pids("http://127.0.0.1:8080", 2, [os.getpid()])
            )
