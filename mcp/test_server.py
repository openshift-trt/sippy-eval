...
class TestPollUrlReady:
    def test_returns_none_when_url_responds(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _poll_url_ready("http://localhost:9999", timeout=5)
            )
    ...
    def test_returns_error_on_timeout(self):
        with mock.patch(
            ...
            result = asyncio.run(
                _poll_url_ready("http://localhost:9999", timeout=2)
            )
    ...
    def test_returns_error_when_pids_die(self):
        ...
        with mock.patch(
            ...
            result = asyncio.run(
                _poll_url_ready("http://localhost:9999", timeout=30, pids=[dead_pid])
            )
            assert result is not None
            assert "exited" in result

    def test_succeeds_even_with_pids(self):
        with mock.patch("server.urllib.request.urlopen"):
            result = asyncio.run(
                _poll_url_ready(
