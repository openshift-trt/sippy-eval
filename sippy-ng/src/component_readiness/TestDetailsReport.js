...
export default function TestDetailsReport(props) {
  ...
  let sippyNgPrefixLen = 10 // length of '/sippy-ng/'
  if (sippyNgIndex === -1) {
    sippyNgIndex = currentUrl.indexOf('/sippy-ng')
    sippyNgPrefixLen = 9 // length of '/sippy-ng'
  }
  let testDetailsApiCall
  if (sippyNgIndex !== -1) {
    const pathAfterSippyNg = currentUrl.substring(
      sippyNgIndex + sippyNgPrefixLen
    )
