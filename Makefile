...
mcp-test: mcp/.venv/.dev-installed
	cd mcp; .venv/bin/pytest test_server.py -v

mcp/.venv/.dev-installed: mcp/dev-requirements.txt mcp/requirements.txt
	uv venv mcp/.venv --allow-existing -q
	uv pip install --python mcp/.venv/bin/python -r mcp/dev-requirements.txt -q
	@touch $@
