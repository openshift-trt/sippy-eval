...
mcp-venv: mcp/.venv/.deps-installed

mcp/.venv/.deps-installed: mcp/requirements.txt
	cd mcp && python3 -m venv .venv
	cd mcp && .venv/bin/pip install --upgrade pip -q
	cd mcp && .venv/bin/pip install -r requirements.txt -q
	touch $@
