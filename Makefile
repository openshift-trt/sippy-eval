...
ifeq ($(ARTIFACT_DIR),)
	mcp/.venv/bin/pytest mcp/test_server.py -v --junitxml=./mcp-junit.xml
else
	mcp/.venv/bin/pytest mcp/test_server.py -v --junitxml=$(ARTIFACT_DIR)/mcp-junit.xml
endif
...
mcp-venv: mcp/.venv/.test-deps

mcp/.venv/.test-deps: mcp/requirements-test.txt mcp/requirements.txt
